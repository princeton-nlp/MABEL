...
def split_into_segments(
    ...
    while curr_idx < len(document_state.subtokens):
        ...
        if end_idx < curr_idx:
            logger.info(
                f"{document_state.doc_key}: no sentence end found; split at token end"
            )
...
def get_document(doc_key, doc_lines, language, seg_len, tokenizer):
    ...
    split_into_segments(
        document_state, seg_len, constraits1, document_state.token_end, tokenizer
    )
...
def minimize_partition(partition, extension, args, tokenizer):
    input_path = os.path.join(
        args.input_dir, f"{partition}.{args.language}.{extension}"
    )
    ...
    with open(output_path, "w") as output_file:
        for doc_key, doc_lines in documents:
            ...
            document = get_document(
                doc_key, doc_lines, args.language, args.seg_len, tokenizer
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--language", type=str, default="english", help="english, chinese, arabic"
    )
